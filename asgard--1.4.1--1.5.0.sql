\echo Use "ALTER EXTENSION asgard UPDATE TO '1.5.0'" to load this file. \quit
-- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
--
-- ASGARD - Système de gestion des droits pour PostgreSQL, version 1.5.0
-- > Script de mise à jour depuis la version 1.4.1
--
...
--
-- Note de version :
-- https://snum.scenari-community.org/Asgard/Documentation/co/SEC_1-5-0.html
-- 
-- Documentation :
...
--
-- Asgard 1.5.0 ajuste certains mécanismes d'Asgard pour assurer la
-- compatibilité de l'extension avec PostgreSQL 16 et 17 :
-- - Des paramètres à renseigner dans la nouvelle table de configuration
--   z_asgard_admin.asgard_configuration permettent d'autoriser les rôles de
--   connexion d'une part, et rôles membres de g_admin d'autre part, 
--   à devenir producteurs de schémas.
-- - C'est désormais un déclencheur exécuté avec les droits
--   du super-utilisateur qui a activé l'extension qui veille, autant que
--   possible, à rendre g_admin membre de tous les rôles producteurs de
--   schémas. Par défaut, il recevra de plus l'option ADMIN sur ces rôles.
--   Des paramètres permettent de ne pas conférer l'option ADMIN ou de ne
--   pas du tout conférer de permission à g_admin sur les producteurs.
--   La fonction z_asgard.asgard_nettoyage_roles() peut être utilisée
--   pour rajouter a posteriori les permissions de g_admin manquantes
--   sur les producteurs des schémas.
-- - Le schéma z_asgard_admin ne peut plus être référencé dans la table
--   de gestion d'Asgard, et sera déréférencé par le script de mise à jour
--   le cas échéant. Une conséquence annexe est qu'il n'est plus possible
--   d'appliquer la fonction de diagnostic à ce schéma. La fonction de 
--   réinitialisation des droits z_asgard.asgard_initialise_schema reste
--   par contre utilisable.
-- - Les déclencheurs sur évènements d'Asgard veillent désormais à ce que les
--   propriétaires des objets d'Asgard ne soient pas modifiés.
-- - g_admin dispose désormais de l'option ADMIN sur tous les rôles d'Asgard.
-- - Le rôle g_admin n'est plus créé par l'extension. Le script d'installation
--   de l'extension renverra une erreur s'il n'a pas été créé au préalable et
--   ne dispose pas du privilège CREATE avec option GRANT sur la base courante.
--   Ce changement est nécessaire pour assurer que le rôle g_admin soit 
--   utilisable et (après désinstallation de l'extension et suppression/
--   réaffectation de tous ses privilèges) puisse être supprimé par l'administrateur 
--   de la base.
-- - Si le rôle qui procède à la mise à jour de l'extension n'est pas un 
--   super-utilisateur et qu'aucun rôle n'est membre de g_admin, 
--   le script de mise à jour fera en sorte que ce rôle devienne membre
--   de g_admin avec l'option ADMIN.
-- - Ajustement de la fonction "asgard_all_login_grant_role",
--   considérant que disposer de l'attribut CREATEROLE n'est plus une condition
--   suffisante pour pouvoir exécuter cette fonction sous PostgreSQL 16+.
-- - Afin de permettre des opérations qui échoueraient autrement
--   sous PostgreSQL 16+, Asgard pourra ajouter les options SET et INHERIT
--   à des rôles qui disposaient déjà de l'option ADMIN sur des rôles dont
--   les privilèges sont requis. Il utilise pour ce faire uniquement les 
--   droits dont dispose le rôle courant, qui est nécessairement membre de 
--   tous les rôles concernés.
-- - Prise en compte de l'ajout du privilège MAINTAIN pour les tables (PostgreSQL 17).
--
-- Au-delà de l'ajout de z_asgard_admin, les "schémas système" au sens d'Asgard,
-- c'est-à-dire les schémas qui ne peuvent pas être référencés dans la table
-- de gestion, incluent désormais tous les schémas dont le nom commence par le
-- préfixe "pg_" et n'incluent plus le schéma topology de PostGIS, dont le
-- référencement ne pose pas de problème particulier.
--
-- Correction de la fonction "asgard_all_login_grant_role", qui ne fonctionnait
-- pas avec des rôles dont le nom n'est pas normalisé.
--
...
BEGIN
    ...
    THEN
        ...
        IF '@extowner@' IN (
            SELECT pg_roles.oid::regrole::text
                FROM pg_roles
                WHERE pg_roles.rolcreaterole 
                    AND NOT pg_roles.rolsuper
        )
        AND NOT 'g_admin' IN (
            SELECT pg_auth_members.roleid::regrole::text 
                FROM pg_auth_members
        )
        THEN

            GRANT g_admin TO @extowner@ WITH ADMIN OPTION ;
    ...
    IF 'g_consult' IN (SELECT rolname FROM pg_catalog.pg_roles)
    THEN
    ...
    IF 'consult.defaut' IN (SELECT rolname FROM pg_catalog.pg_roles)
    THEN
...
SELECT z_asgard_admin.asgard_sortie_gestion_schema('z_asgard_admin') ;

ALTER TABLE z_asgard_admin.gestion_schema
    DROP CONSTRAINT gestion_schema_no_system_schema,
    ADD CONSTRAINT gestion_schema_aucun_schema_systeme CHECK (
...
DROP FUNCTION z_asgard.asgard_expend_privileges(text) ;
...
DROP TRIGGER asgard_visibilite_admin_after
    ON z_asgard_admin.gestion_schema ;

DROP FUNCTION z_asgard_admin.asgard_visibilite_admin_after() ;
