...
BEGIN
    ...
    IF NOT has_database_privilege('g_admin', current_database(), 'CREATE')
    THEN
    
        EXECUTE 'GRANT CREATE ON DATABASE ' || current_database() || ' TO g_admin' ;
...
CREATE TABLE z_asgard_admin.asgard_parametre
(
    id serial PRIMARY KEY,
    connex_param text[] DEFAULT ARRAY['host', 'port', 'dbname', 'sslmode'],
    sslmode text COLLATE pg_catalog."default" NOT NULL DEFAULT 'require',
    nom_service text DEFAULT NULL
    CONSTRAINT asgard_parametre_id_check CHECK (id = 1),
    CONSTRAINT asgard_parametre_sslmode_check CHECK (sslmode = ANY (ARRAY['disable', 'allow', 'prefer', 'require', 'verify-ca', 'verify-full'])),
    CONSTRAINT asgard_parametre_connex_param CHECK (((connex_param) <@ ARRAY['host', 'port', 'dbname', 'sslmode', 'user'] OR connex_param IS NULL) AND (nom_service IS NOT NULL OR ARRAY['host', 'port', 'dbname', 'sslmode'] <@ (connex_param)))
)
WITH (
    OIDS = FALSE
)
...
ALTER TABLE z_asgard_admin.asgard_parametre
    OWNER to g_admin ;

GRANT SELECT ON TABLE z_asgard_admin.asgard_parametre TO g_admin_ext ;

COMMENT ON TABLE z_asgard_admin.asgard_parametre IS 'ASGARD. Table de paramétrage.' ;

COMMENT ON COLUMN z_asgard_admin.asgard_parametre.id IS 'Clé primaire entière.' ;
COMMENT ON COLUMN z_asgard_admin.asgard_parametre.sslmode IS 'Vue pour MenuBuilder. Mode SSL à faire apparaître dans les chaînes de connexion. Ce paramètre est ignoré pour les connexions locales, qui sont toujours établies avec ssl_mode=''disable''.' ;
COMMENT ON COLUMN z_asgard_admin.asgard_parametre.nom_service IS 'Vue pour MenuBuilder. Si les connexions sont définies par un fichier pg_service.conf, nom du "service" à faire apparaître dans les chaînes de connexion.' ;
COMMENT ON COLUMN z_asgard_admin.asgard_parametre.connex_param IS 'Vue pour MenuBuilder. Liste des paramètres spécifiés par la configuration de la connexion dans QGIS (= ceux qui apparaissent dans les chaînes de connexion), parmi ''host'', ''port'', ''dbname'', ''sslmode'', ''user''.' ;

-- la table est marquée comme table de configuration de l'extension
SELECT pg_extension_config_dump('z_asgard_admin.asgard_parametre'::regclass, '') ;
...
ALTER VIEW z_asgard.qgis_menubuilder_metadata
    OWNER TO g_admin_ext ;
    
GRANT SELECT ON TABLE z_asgard.qgis_menubuilder_metadata TO g_consult ;

COMMENT ON VIEW z_asgard.qgis_menubuilder_metadata IS E'ASGARD. MenuBuilder. Vue appelée par le plugin MenuBuilder de QGIS.
...
COMMENT ON COLUMN z_asgard.qgis_menubuilder_metadata.id IS 'Identifiant entier unique.' ;
COMMENT ON COLUMN z_asgard.qgis_menubuilder_metadata.name IS 'Nom de la table. Le nom de l''éventuel champ de géométrie est précisé si son nom n''est pas geom ([table].[champ de géométrie]).' ;
COMMENT ON COLUMN z_asgard.qgis_menubuilder_metadata.profile IS 'Nom du profil de MenuBuilder. Vaut toujours ''consultation'', la liste des tables étant ajustée automatiquement en fonction des privilèges de l''utilisateur.' ;
COMMENT ON COLUMN z_asgard.qgis_menubuilder_metadata.model_index IS 'Arborescence du menu (à partir des champs niv1 et niv2 de gestion_schema).' ;
COMMENT ON COLUMN z_asgard.qgis_menubuilder_metadata.datasource_uri IS 'Chemin d''accès aux données (connexion string).' ;
COMMENT ON COLUMN z_asgard.qgis_menubuilder_metadata.table_comment IS 'Le cas échéant, commentaire saisi sur la table.' ;
